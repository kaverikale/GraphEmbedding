...
from graph_embedding.utils import *
#from graph_embedding.eval_metrics import eval_metrics
...
def LinkPrediction(embedding_look_up, original_graph, train_graph, test_pos_edges, seed):
    ...
    for edge in test_pos_edges:
        ...
        node_v_emb = embedding_look_up[edge[1]]
        feature_vector = np.append(node_u_emb, node_v_emb)
        text_edges.append((edge[0],edge[1]))
    ...
    c = list(zip(X_test, y_test, text_edges))
    random.shuffle(c)
    X_test, y_test, text_edges = zip(*c)
    ...
    #MAP 
    #test_pos_edges vs predicted edges
    y_pred_proba, y_pred,  text_edges, y_test = zip(*sorted(zip(y_pred_proba, y_pred,  text_edges, y_test), key = lambda x: -x[0]))
    print(y_pred_proba, y_pred, y_test)
    MAP = 0# calculateMAP(text_edges, y_test, y_pred, original_graph, 10)
    MRR = 0#computeMRR(text_edges, y_test, y_pred, original_graph, 10)
    print('#' * 9 + ' Link Prediction Evaluation ' + '#' * 9)
    print(f'MAP : {MAP:.3f}, MRR : {MRR:.3f}, AUC-ROC: {auc_roc:.3f}, AUC-PR: {auc_pr:.3f}, Accuracy: {accuracy:.3f}, F1: {f1:.3f}')
    print('#' * 50)
