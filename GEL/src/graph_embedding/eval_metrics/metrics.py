...
def computePrecisionCurveAndRank(predicted_edge_list, true_digraph, max_k=-1):
    if max_k == -1:
...
def computeMRR(predicted_edge_list, true_digraph, max_k=-1):
    ...
    for i in range(node_num):
        if true_digraph.out_degree(i) == 0:
            continue
        count += 1
        _, delta_factors = computePrecisionCurveAndRank(node_edges[i], true_digraph, max_k)
...
def computeMAP(predicted_edge_list, true_digraph, max_k=-1):
    ...
    for i in range(node_num):
        if true_digraph.out_degree(i) == 0:
            continue
        count += 1
        precision_scores, delta_factors = computePrecisionCurveAndRank(node_edges[i], true_digraph, max_k)
